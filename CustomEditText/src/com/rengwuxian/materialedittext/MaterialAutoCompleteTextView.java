...
public class MaterialAutoCompleteTextView extends AutoCompleteTextView {
	...
	public MaterialAutoCompleteTextView(Context context, AttributeSet attrs,
		...
		floatingLabelTextSize = getResources().getDimensionPixelSize(
		...
		try {
			...
			try {
				int colorAccentId = getResources().getIdentifier(
						"colorPrimary", "attr", getContext().getPackageName());
				if (colorAccentId != 0) {
					context.getTheme().resolveAttribute(colorAccentId,
							primaryColorTypedValue, true);
					defaultPrimaryColor = primaryColorTypedValue.data;
				} else {
		...
		primaryColor = typedArray.getColor(
				R.styleable.MaterialEditText_primaryColor, defaultPrimaryColor);
	...
	private void initPadding() {
		...
		extraPaddingBottom = (int) ((fontMetrics.descent - fontMetrics.ascent) * currentBottomLines)
				+ (hideUnderline ? bottomSpacing : bottomSpacing * 2);
		setPaddings(getPaddingLeft(), paddingTop, getPaddingRight(),
				paddingBottom);
	}
	...
	private void adjustBottomLines() {
		...
		if (tempErrorText != null) {
			textLayout = new StaticLayout(tempErrorText, textPaint,
					getMeasuredWidth() - getBottomTextLeftOffset()
							- getBottomTextRightOffset(),
					Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
			destBottomLines = Math.max(textLayout.getLineCount(),
					minBottomTextLines);
		} else if (helperText != null) {
			textLayout = new StaticLayout(helperText, textPaint,
					getMeasuredWidth() - getBottomTextLeftOffset()
							- getBottomTextRightOffset(),
					Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
			destBottomLines = Math.max(textLayout.getLineCount(),
	...
	private void initFloatingLabel() {
		if (floatingLabelEnabled) {
			...
			if (highlightFloatingLabel) {
				// observe the focus state to animate the floating label's text
				// color appropriately
	...
	protected void onDraw(@NonNull Canvas canvas) {
		...
		if (!hideUnderline) {
			...
			if (!isInternalValid()) { // not valid
				...
				canvas.drawRect(getScrollX(), lineStartY, getWidth()
						+ getScrollX(), lineStartY + getPixel(2), paint);
			} else if (!isEnabled()) { // disabled
				paint.setColor(baseColor & 0x00ffffff | 0x44000000);
				float interval = getPixel(1);
				for (float startX = 0; startX < getWidth(); startX += interval * 3) {
					canvas.drawRect(getScrollX() + startX, lineStartY,
							getScrollX() + startX + interval, lineStartY
									+ getPixel(1), paint);
				}
				...
				paint.setColor(baseColor & 0x00ffffff | 0x44000000);
				canvas.drawRect(getScrollX(), lineStartY, getWidth()
						+ getScrollX(), lineStartY + getPixel(1), paint);
			}
		}

		float relativeHeight = -fontMetrics.ascent - fontMetrics.descent;
		float fontPaddingTop = floatingLabelTextSize + fontMetrics.ascent
				+ fontMetrics.descent;
		...
		if ((hasFocus() && hasCharatersCounter()) || !isCharactersCountValid()) {
			...
			canvas.drawText(text,
					getWidth() + getScrollX() - textPaint.measureText(text),
		...
		if (textLayout != null) {
			...
			if (tempErrorText != null) { // validation failed
				...
				canvas.save();
				canvas.translate(bottomTextStartX, lineStartY + bottomSpacing
						- fontPaddingTop);
				textLayout.draw(canvas);
				canvas.restore();
			} else if (hasFocus() && !TextUtils.isEmpty(helperText)) {
				textPaint.setColor(helperTextColor != -1 ? helperTextColor
		...
		if (floatingLabelEnabled && !TextUtils.isEmpty(floatingLabelText)) {
			...
			if ((getGravity() & Gravity.LEFT) == Gravity.LEFT) {
				...
				floatingLabelStartX += getWidth() - getPaddingRight()
						- floatingLabelWidth;
			} else {
				floatingLabelStartX += (int) (getPaddingLeft() + (getWidth()
						- getPaddingLeft() - getPaddingRight() - floatingLabelWidth) / 2);
			}
		...
		if (hasFocus() && singleLineEllipsis && getScrollX() != 0) {
			...
			canvas.drawCircle(bottomEllipsisSize / 2 + getScrollX(), startY
					+ bottomEllipsisSize / 2, bottomEllipsisSize / 2, paint);
			canvas.drawCircle(bottomEllipsisSize * 5 / 2 + getScrollX(), startY
					+ bottomEllipsisSize / 2, bottomEllipsisSize / 2, paint);
			canvas.drawCircle(bottomEllipsisSize * 9 / 2 + getScrollX(), startY
					+ bottomEllipsisSize / 2, bottomEllipsisSize / 2, paint);
		}
